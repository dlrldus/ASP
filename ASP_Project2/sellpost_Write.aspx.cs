    ...
    protected void btnWrite_Click(object sender, EventArgs e)
        ...
        if (ImageUpload.FileName != "") // 이미지가 존재하지 않는다면, 업로드 불가
            ...
            if (Book_status.SelectedIndex == 0) // 책의 품질상태를 정하지않을경우 업로드 불가
                ...
                string InsertQuery = "INSERT INTO sellpost (name,email,title,book,contents,writedate,readcount,image,price,pay_code,status,Category_f,Category_s,latitude,longitude) values(@name,@email,@title,@book,@contents,@writedate,0,@image,@price,@pay_code,@status,@Category_f,@Category_s,@latitude,@longitude)";
                ...
                cmd.Parameters.AddWithValue("@book", bookname.Text);          
